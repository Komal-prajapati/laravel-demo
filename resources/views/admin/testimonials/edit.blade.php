            ...
            <div class="bg-white shadow-sm overflow-hidden sm:rounded-lg">
                @if ($testimonial->person_avatar)
                    <div class="flex items-center justify-center">
                        <img src="{{ asset('storage' . $testimonial->person_avatar) }}" alt="{{ $testimonial->person_name }}" class="w-36 h-36 my-4 rounded">
                    </div>
                @endif
                ...
                <form action="{{ route('admin.testimonials.update', $testimonial->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="px-6 py-4 grid grid-col-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <x-input id="person_name" class="block mt-1 w-full" type="text" name="person_name" :value="$testimonial->person_name" required autofocus />
                        </div>
                        <div>
                            <x-label for="company_name" :value="__('Company Name')" />
                            <x-input id="company_name" class="block mt-1 w-full" type="text" name="company_name" :value="$testimonial->company_name" />
                        </div>
                    </div>
                    ...
                    <div class="px-6 py-4">
                        ...
                        <textarea rows="5" name="content" id="content" class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 block mt-1 w-full" required>{{ $testimonial->content }}</textarea>
                    </div>
